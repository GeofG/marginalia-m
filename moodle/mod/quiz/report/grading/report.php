...
class quiz_grading_report extends quiz_default_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        if (!$hasquestions) {
			...
			// #marginalia begin
			// Oh isn't this just peachy. Moodle sets the URL not to this
			// page (with slot and qid set), but to the preceding page,
			// without those set. So I need to pass those values in.
			// I could explicitly construct the correct profile, but then I'm
			// making this code vulnerable to how that's done. Instead construct
			// the *correct* URL for this page, and use that.
			$moodlemia = moodle_marginalia::get_instance( );
			// WRONG: $PAGE->url->out( false ) );
			$miaprofile = $moodlemia->get_profile( $_SERVER['REQUEST_URI'] );
			// The following is wrong because it's not a quiz ID.
			//$mia_url = "/mod/quiz/report.php?id=".$quiz->id."&mode=grading"
			//	."&slot=".$slot."&qid=".$questionid;
			if ($miaprofile) {
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            ...
            return '1x' . get_string('gradingattempt', 'quiz_grading', $a);
        } else if ($showidnumbers) {
            $a->fullname = $attempt->idnumber;
            return '2x' . get_string('gradingattempt', 'quiz_grading', $a);
        } else {
            return '';
        }
    }
