	...
	switch ( pageName ) {
		...
		case '/mod/quiz/report':
			var selectors = {
				...
				// Selector's first argument must return element list, so...
				post_url: new Selector( function( root ) { return [root] }, 
					null, function( root ) {
						var inputs = $( "input[type='hidden']", root);
						for ( var i = 0; i < inputs.length; ++i ) {
							var node = inputs[ i ];
							// Kind of hacky, as this might change. Currentyl, Moodle
							// spits out <input name="q8:1_:sequencecheck" value="3" type="hidden"/>
							var m = node.name.match( /(\d+):(\d+)_:sequencecheck/ );
							// I'm not sure if the step is necessary
			...
			$( '.que.essay .ablock .answer div' ).after(
				'<ol class="mia_margin"><li class="mia_dummyfirst">dummy</li></ol>');
